/*!
 * \file dll_pll_fll_veml_tracking.cc
...
#include "dll_pll_fll_veml_tracking.h"
...
dll_pll_fll_veml_tracking_sptr dll_pll_fll_veml_make_tracking(dllpllfllconf_t conf_)
{
    return dll_pll_fll_veml_tracking_sptr(new dll_pll_fll_veml_tracking(conf_));
}


void dll_pll_fll_veml_tracking::forecast(int noutput_items,
...
dll_pll_fll_veml_tracking::dll_pll_fll_veml_tracking(dllpllfllconf_t conf_) : gr::block("dll_pll_fll_veml_tracking", gr::io_signature::make(1, 1, sizeof(gr_complex)),
                                                                       gr::io_signature::make(1, 1, sizeof(Gnss_Synchro)))
{
    ...
    d_carrier_loop_filter.set_params(trk_parameters.fll_bw_hz, trk_parameters.pll_bw_hz, 3);
...
void dll_pll_fll_veml_tracking::start_tracking()
{
    ...
    d_carrier_loop_filter.initialize(d_acq_carrier_doppler_hz);     // Initialize FLL filter
...
dll_pll_fll_veml_tracking::~dll_pll_fll_veml_tracking()
{
...
bool dll_pll_fll_veml_tracking::acquire_secondary()
{
...
bool dll_pll_fll_veml_tracking::cn0_and_tracking_lock_status(double coh_integration_time_s)
{
...
void dll_pll_fll_veml_tracking::do_correlation_step(const gr_complex *input_samples)
{
...
void dll_pll_fll_veml_tracking::run_dll_pll_fll()
{
    // ################## PLL ##########################################################
    // PLL discriminator
/*  if (d_cloop)
...
*/
    ...
    if(d_FLL_wait == 1)
    {
        d_Prompt_prev = d_P_accu;
        d_FLL_wait = 0;
    }else{
        d_FLL_discriminator_hz = fll_four_quadrant_atan(d_Prompt_prev, d_P_accu, 0, correlation_time_s) / PI_2;

        d_Prompt_prev = d_P_accu;
        d_FLL_wait = 1;
    }
    // Should be rad actually
    PLL_discriminator_hz = pll_cloop_two_quadrant_atan(d_P_accu) / PI_2;
...
void dll_pll_fll_veml_tracking::clear_tracking_vars()
{
    std::fill_n(d_correlator_outs, d_n_correlator_taps, gr_complex(0.0, 0.0));
...
void dll_pll_fll_veml_tracking::update_tracking_vars()
{
    ...
    d_pllfllces = static_cast<int>(round(K_blk_samples));  // round to a discrete number of samples
...
void dll_pll_fll_veml_tracking::save_correlation_results()
{
    if (d_secondary)
        {
...
void dll_pll_fll_veml_tracking::log_data(bool integrating)
{
    if (trk_parameters.dump)
        {
...
int dll_pll_fll_veml_tracking::save_matfile()
{
    // READ DUMP FILE
...
void dll_pll_fll_veml_tracking::set_channel(unsigned int channel)
{
    gr::thread::scoped_lock l(d_setlock);
    d_channel = channel;
...
void dll_pll_fll_veml_tracking::set_gnss_synchro(Gnss_Synchro *p_gnss_synchro)
{
    gr::thread::scoped_lock l(d_setlock);
...
int dll_pll_fll_veml_tracking::general_work(int noutput_items __attribute__((unused)), gr_vector_int &ninput_items,
